...
describe("container", () => {
  it("renderer", () => {
    const markdownIt = MarkdownIt({ linkify: true }).use(container, "spoiler", {
      render: (tokens, idx) =>
  ...
  describe("validator", () => {
    it("should skip rule if return value is falsy", () => {
      const markdownIt = MarkdownIt({ linkify: true }).use(container, "name", {
        validate: () => false,
      });
    ...
    it("rule should call it", () => {
      ...
      markdownIt.parse(":\n::\n:::\n::::\n:::::\n", {});
      expect(vi.fn).toBeCalledTimes(3);
    });
